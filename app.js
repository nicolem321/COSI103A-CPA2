...
const stations = require('./public/data/stations.json')
...
const mongodb_URI = 'mongodb+srv://nicolem321:Mengziyi20010321@cluster0.gmlcx.mongodb.net/myFirstDatabase?retryWrites=true&w=majority'
...
app.get('/upsertDB',
  async (req,res,next) => {
    ...
    for (station of stations){
      const {stationId,nameEng,lineNameEng,lineColorHex}=station;
      //const num = getNum(coursenum);
      //course.num=num
      //course.suffix = coursenum.slice(num.length)
      await Station.findOneAndUpdate({stationId,nameEng,lineNameEng,lineColorHex},station,{upsert:true})
    }
    const num = await Station.find({}).count();
    res.send("data uploaded: "+num)
  }
)


app.post('/stations/byId',
  // show list of courses in a given subject
  async (req,res,next) => {
    const {stationId} = req.body;
    const station = await Station.find({stationId:stationId})
    //let items = await Station.remove({})

    // for (let i = 0; i < stations.length; i++) {
    //   const stationId = stations[i]['stationId']
    //   const name = stations[i]['name']
    //   const nameEng = stations[i]['nameEng']
    //   const geoLat = stations[i]['getLat']
    //   const geoLng = stations[i]['geoLng']
    //   const lineName = stations[i]['lineName']
    //   const lineNameEng = stations[i]['lineNameEng']
    //   const lineColorHex = stations[i]['lineColorHex']
    //   const lineServiceName = stations[i]['lineServiceName']
    //   //console.log("Here");
    ...
    //     lineNameEng,
    //     lineColorHex,
    //     lineServiceName} // create the data object
    //   let item = new Station(data) // create the database object (and test the types are correct)
    //   await item.save()
    // }
    res.locals.stations = station
